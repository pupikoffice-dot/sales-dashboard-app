...
import { getDualMonthCols, sumMonthRows } from '../../lib/salesDateFilter'
...
function groupTotals(group: DualMonthGroup) {

  return {

    cash: group.currentRows.reduce((a, r) => a + (r.cash || 0), 0),

    qty: group.currentRows.reduce((a, r) => a + (r.qty || 0), 0),
...
export function DualMonthGroupedTable({
  ...
  const body = visibleGroups.map(group => {
    ...
    const monthCells = cols.map(dc => {
      ...
      const cur = sumMonthRows(group.currentRows, dc.cur, dc.m)

      const prev = sumMonthRows(group.compareRows, dc.prev, dc.m)
